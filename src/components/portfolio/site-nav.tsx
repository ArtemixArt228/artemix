...
export function SiteNav({ overlay = false }: { overlay?: boolean }) {
	return (
		...
		>
			...
			>
				<span className="portfolio-pulse inline-block size-2 rounded-full bg-brand shadow-[0_0_12px_#cc6437]" />
				<span>
					Artem Kovalitskyi{" "}
			...
			<div className="flex items-center gap-3">
				<Badge
					className={cn(
						"h-auto gap-1.5 rounded-pill border-white/35 bg-transparent px-[11px] py-[5px] font-cond text-[14px] uppercase leading-[0.9] tracking-[-0.02em] text-white",
						overlay && "[text-shadow:0_1px_8px_rgba(0,0,0,0.6)]",
					)}
				>
					<span className="portfolio-pulse size-1.5 rounded-full bg-brand shadow-[0_0_8px_#cc6437]" />
					Available · Jun 2026
				</Badge>
				<Button
					variant="outline"
